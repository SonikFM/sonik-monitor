...
interface InfiniteListRowProps {
    ...
    selectedProperties: Record<TaxonomicFilterGroupType, (string | number)[]>
    eventNames: string[]
    index: number
    isActiveTab: boolean
    ...
    if (showNonCapturedEventOption && rowIndex === 0) {
        const selectNonCapturedEvent = (): void => {
            selectItem(
                itemGroup,
                trimmedSearchQuery,
                { name: trimmedSearchQuery, isNonCaptured: true } as TaxonomicDefinitionTypes,
                trimmedSearchQuery
            )
        }
...
export function InfiniteList({ popupAnchorElement }: InfiniteListProps): JSX.Element {
    ...
    const rowProps: InfiniteListRowProps = {
        results,
        taxonomicGroups,
        group,
        listGroupType,
